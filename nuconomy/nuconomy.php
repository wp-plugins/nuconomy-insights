...
define(NUCONOMY_STR_GERBIL_APPROVE,'Once our Team of Mighty Gerbils approves of your request, you will recive another e-mail containing activation instructions.');
...
function nucon_addjs() {
    ...
    if ($project && !is_404()) { // dont add js when we didnt  finish registretion or for a 404 page
      echo "\n";
...
?>
<?php if (is_single()) { // its pointless to check the cookie for any other type of page ?>
<?php } ?>
...
<?php
        ...
        ?>
NUCONOMY.ContentTitle = "<?php echo $title; ?>";
NUCONOMY.Tags = "<?php echo nucon_get_tags();?>";
<?php if (is_single()) { // its pointless to check the cookie for any other type of page ?>
        if (hasNucRSSCookie('<?php echo NUCONOMY_RSS_COOKIE; ?>')) {
<?php echo "NUCONOMY.Logger.LogActivity(\"$user\",\"$page_owner\",\"$page\",".NUCONOMY_RSS_HIT.');';?>
        } 
<?php } ?>
<?php echo "NUCONOMY.Logger.LogActivity(\"$user\",\"$page_owner\",\"$page\",".nucon_get_hit_type().');';?>

<?php
      $nucon_search_results = nucon_get_search_results();
	  foreach ($nucon_search_results as $v) {
	      echo 'NUCONOMY.ContentType = "' . $v['type']."\"\n";
	      echo 'NUCONOMY.ContentTitle = "'.$v['title']."\"\n";
	      echo 'NUCONOMY.Tags = "'.$v['tags']."\"\n";
	      $url = $v['url'];
	      $page_owner=$v['owner'];
		  echo "NUCONOMY.Logger.LogActivity(\"$user\",\"$page_owner\",\"$url\",".NUCONOMY_SEARCH_HIT.');'."\n";
	  }
      echo "\n</script>\n";
  ...
  if ($nucon_is_mu) {
	 if (is_site_admin()) {
		if (nucon_get_project_client_token() && !isset($_POST['nucon_reset']) ) {
			...
			add_submenu_page(basename(__FILE__), NUCONOMY_STR_CONFIG, NUCONOMY_STR_INSIGHTS_SETTINGS, 10, 'nuconomy/admin.php', 'nucon_config_page');
		} else {
			add_menu_page(NUCONOMY_STR_INSIGHTS, NUCONOMY_STR_INSIGHTS, 10, 'nuconomy/admin.php','nucon_config_page');
			add_submenu_page(basename(__FILE__), NUCONOMY_STR_CONFIG, NUCONOMY_STR_INSIGHTS_SETTINGS, 10, 'nuconomy/admin.php', 'nucon_config_page');
		}
	 }
  } else {
    load_plugin_textdomain('nuconomy', 'wp-content/plugins/nuconomy');
    if (nucon_get_project_client_token() && !isset($_POST['nucon_reset']) ) {
	    ...
	    add_submenu_page(__FILE__, NUCONOMY_STR_CONFIG, NUCONOMY_STR_INSIGHTS_SETTINGS, 10, 'nuconomy/admin.php', 'nucon_config_page');
	} else {
        add_menu_page(NUCONOMY_STR_INSIGHTS, NUCONOMY_STR_INSIGHTS, 10, 'nuconomy/admin.php','nucon_config_page');
	    add_submenu_page('nuconomy/admin.php', NUCONOMY_STR_CONFIG, NUCONOMY_STR_INSIGHTS_SETTINGS, 10, 'nuconomy/admin.php', 'nucon_config_page');
	}
...
function nucon_config_page() {
 ...
 } else if (isset($_GET['fintok']) && (nucon_get_option('nuconomy_step') == '2')) {
      if (!current_user_can('manage_options')) die(__('You should log on as administrator.'));
      if (nucon_get_codes($_GET['fintok'],$_GET['contactEmail'])) {
...
function nucon_not_reg_warning() {
  global $nucon_is_mu;
	...
	if (!nucon_get_option('nuconomy_step') && !nucon_get_project_server_token() && ($_GET['page'] != 'nuconomy.php') && ($_GET['page'] != 'nuconomy/nuconomy.php')  && ($_GET['page'] != 'nuconomy/admin.php')) {
        if ($nucon_is_mu) {
          if (is_site_admin())
		        echo "	<div class='updated fade'><p><strong>".NUCONOMY_STR_NOT_REGISTERED."</strong> ".sprintf(NUCONOMY_STR_MUST_REGISTER, "wpmu-admin.php?page=nuconomy.php")."</p></div>
		    ";
        } else
...
function nucon_replace_youtubeobject($matches) {
  ...
  $ret = '<div id="nuconyt'.$nucon_youtube_index.'">You need Flash player 8+ and JavaScript enabled to view this video.</div>';
  $ret .= '<script type="text/javascript">';
  $ret .= 'swfobject.embedSWF("'.$t['src'].'&enablejsapi=1&playerapiid=player'.$nucon_youtube_index.'",' .
                       '"'.nuconyt.$nucon_youtube_index.'", "'.$t['width'].'", "'.$t['height'].'", "8", null, null,' . 
                       '{ allowScriptAccess: "always" },' .
                       '{ id: "player'.$nucon_youtube_index.'" });';
  $ret .= '</script>';
  $nucon_youtube_index++;
  return $ret;
...
function nucon_convertvideo($content) {
  ...
  if (!nucon_get_project_client_token())
    return $content;
...
function nucon_add_head_js() {
    ...
    if ($project) {
      if ($nucon_is_mu)
        echo '<script src="'.get_option('siteurl') . '/' . MUPLUGINDIR . '/nuconomy/NuconomySafeApi.js" type="text/javascript"></script>';
      else
        echo '<script src="'.get_option('siteurl') . '/wp-content/plugins/nuconomy/NuconomySafeApi.js" type="text/javascript"></script>';
?>
...
<?php         
        if ($nucon_is_mu) {
          echo '<script src="'.get_option('siteurl') . '/' . MUPLUGINDIR . '/nuconomy/swfobject.js" type="text/javascript"></script>'."\n";
          echo '<script src="'.get_option('siteurl') . '/' . MUPLUGINDIR . '/nuconomy/nuconomyutils.js" type="text/javascript"></script>'."\n";
        } else {
          echo '<script src="'.get_option('siteurl') . '/wp-content/plugins/nuconomy/swfobject.js" type="text/javascript"></script>'."\n";
          echo '<script src="'.get_option('siteurl') . '/wp-content/plugins/nuconomy/nuconomyutils.js" type="text/javascript"></script>'."\n";
        }
   }
}
...
function nucon_load_nusoap() {
  ...
  if ($nucon_is_mu) {
	  require_once(ABSPATH . MUPLUGINDIR . '/nuconomy/nusoap/lib/nusoap.php');
  } else {
	  require_once(ABSPATH .'wp-content/plugins/nuconomy/nusoap/lib/nusoap.php');
  }
}
